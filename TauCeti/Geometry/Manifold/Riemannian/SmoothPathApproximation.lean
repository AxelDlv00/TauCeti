...
public import TauCeti.Geometry.Manifold.Riemannian.CurveReadback

/-!
# Piecewise `C¹` competitors for the Riemannian distance

This module records the source-backed chart-polygon approximation using Mathlib's canonical
`RiemannianBundle` and `Manifold.pathELength` APIs.

The source contract is the Apache-2.0 Poincare-Conjecture snapshot at revision
`24f32e4d600878bfaac6bc2f2f9324175571c321` (`[poincareConjectureDoCarmo]`), specifically
`DoCarmoLib/Riemannian/Metric/SmoothPathApproximation.lean` /
`Riemannian.exists_piecewiseSmooth_pathELength_le`. The public approximation has the same
endpoint, partition, smooth-piece, and multiplicative length statements after adapting the
canonical metric API. Tau Ceti's existing `EDistComparison` module already supplies the stronger
piecewise-`C¹` infimum comparison through exact corner smoothing, so that API is not duplicated
here.

References: do Carmo, *Riemannian Geometry*, Chapter 3, Definition 3.1 and Chapter 7,
Definition 2.4 (`[doCarmo1992]`), and the `HopfRinow` roadmap Layer 0 target.
-/
...
private theorem chartMetricInner_sq_le_mul_of_mem_target
    (α : M) {y : E} (hy : y ∈ (extChartAt I α).target) (a b : E) :
    chartMetricInner (I := I) α y a b ^ 2 ≤
      chartMetricInner (I := I) α y a a * chartMetricInner (I := I) α y b b := by
  have hquad : ∀ t : ℝ, 0 ≤
      chartMetricInner (I := I) α y b b * (t * t) +
        2 * chartMetricInner (I := I) α y a b * t +
        chartMetricInner (I := I) α y a a := by
    intro t
    have h := chartMetricInner_self_nonneg_of_mem_target (I := I) α hy (a + t • b)
    have hexpand : chartMetricInner (I := I) α y (a + t • b) (a + t • b) =
        chartMetricInner (I := I) α y b b * (t * t) +
          2 * chartMetricInner (I := I) α y a b * t +
          chartMetricInner (I := I) α y a a := by
      ...
      rw [chartMetricInner_symm (I := I) α y b a, chartMetricInner_smul_left]
      ring
    rwa [hexpand] at h
...
private theorem chartMetricInner_le_sqrt_mul_sqrt_of_mem_target
    ...
    chartMetricInner (I := I) α y a b ≤
      Real.sqrt (chartMetricInner (I := I) α y a a) *
        Real.sqrt (chartMetricInner (I := I) α y b b) := by
  have h1 : chartMetricInner (I := I) α y a b ≤
      |chartMetricInner (I := I) α y a b| := le_abs_self _
  have h2 : |chartMetricInner (I := I) α y a b| =
      Real.sqrt (chartMetricInner (I := I) α y a b ^ 2) :=
    (Real.sqrt_sq_eq_abs _).symm
  rw [h2] at h1
  refine h1.trans ?_
  rw [← Real.sqrt_mul
    (chartMetricInner_self_nonneg_of_mem_target (I := I) α hy a)]
  exact Real.sqrt_le_sqrt
    (chartMetricInner_sq_le_mul_of_mem_target (I := I) α hy a b)

/-- The source's local Gram comparison, with the current canonical bundle metric. -/
private theorem exists_chartBall_gram_comparison (p : M) {eta : ℝ} (heta : 0 < eta) :
    ∃ delta > (0 : ℝ),
      closedBall (extChartAt I p p) delta ⊆ (extChartAt I p).target ∧
      ∀ y ∈ closedBall (extChartAt I p p) delta, ∀ u : E,
        (1 - eta) * chartMetricInner (I := I) p (extChartAt I p p) u u ≤
            chartMetricInner (I := I) p y u u ∧
          chartMetricInner (I := I) p y u u ≤
            (1 + eta) * chartMetricInner (I := I) p (extChartAt I p p) u u := by
  classical
  set x0 : E := extChartAt I p p with hx0
  have hx0tgt : x0 ∈ (extChartAt I p).target := mem_extChartAt_target p
  obtain ⟨c, V, hc, hV, -, hcoerc⟩ :=
    Riemannian.exists_sq_norm_le_chartMetricInner (I := I) p
  set lam : ℝ := 1 / c with hlam_def
  have hlam : 0 < lam := by rw [hlam_def]; positivity
  have hlow0 : ∀ u : E, lam * ‖u‖ ^ 2 ≤
      chartMetricInner (I := I) p x0 u u := by
    intro u
    have h := hcoerc x0 (mem_of_mem_nhds hV) u
    rw [hlam_def, one_div, inv_mul_le_iff₀ hc]
    simpa [mul_comm] using h
  set S : E → ℝ := fun y ↦ ∑ i, ∑ j,
    |chartGramOnE (I := I) p i j y - chartGramOnE (I := I) p i j x0| *
      (‖Riemannian.chartCoordFunctional (E := E) i‖ *
        ‖Riemannian.chartCoordFunctional (E := E) j‖) with hS
  have hScont : ContinuousAt S x0 := by
    simp only [hS]
    refine tendsto_finsetSum _ fun i _ ↦ tendsto_finsetSum _ fun j _ ↦ ?_
    have hGij : ContinuousAt (chartGramOnE (I := I) p i j) x0 :=
      ((chartGramOnE_contDiffOn (I := I) p i j).contDiffAt
  ...
  have hcoord : ∀ i : Fin (Module.finrank ℝ E),
      |Riemannian.chartCoord (E := E) i u| ≤
        ‖Riemannian.chartCoordFunctional (E := E) i‖ * ‖u‖ := by
    intro i
    rw [← Riemannian.chartCoordFunctional_apply (E := E) i u, ← Real.norm_eq_abs]
    exact (Riemannian.chartCoordFunctional (E := E) i).le_opNorm u
  have hdev : |chartMetricInner (I := I) p y u u -
      chartMetricInner (I := I) p x0 u u| ≤ S y * ‖u‖ ^ 2 := by
    have hsub : chartMetricInner (I := I) p y u u -
        chartMetricInner (I := I) p x0 u u = ∑ i, ∑ j,
          (chartGramOnE (I := I) p i j y - chartGramOnE (I := I) p i j x0) *
            Riemannian.chartCoord (E := E) i u * Riemannian.chartCoord (E := E) j u := by
      rw [chartMetricInner, chartMetricInner, ← Finset.sum_sub_distrib]
    ...
    rw [hsub]
    calc
      |∑ i, ∑ j, (chartGramOnE (I := I) p i j y - chartGramOnE (I := I) p i j x0) *
          Riemannian.chartCoord (E := E) i u * Riemannian.chartCoord (E := E) j u| ≤
          ∑ i, |∑ j, (chartGramOnE (I := I) p i j y -
            chartGramOnE (I := I) p i j x0) * Riemannian.chartCoord (E := E) i u *
              Riemannian.chartCoord (E := E) j u| := Finset.abs_sum_le_sum_abs _ _
      _ ≤ ∑ i, ∑ j, |(chartGramOnE (I := I) p i j y -
            chartGramOnE (I := I) p i j x0) * Riemannian.chartCoord (E := E) i u *
              Riemannian.chartCoord (E := E) j u| :=
          Finset.sum_le_sum fun i _ ↦ Finset.abs_sum_le_sum_abs _ _
      _ ≤ ∑ i, ∑ j, |chartGramOnE (I := I) p i j y -
            chartGramOnE (I := I) p i j x0| *
          (‖Riemannian.chartCoordFunctional (E := E) i‖ *
            ‖Riemannian.chartCoordFunctional (E := E) j‖) * ‖u‖ ^ 2 := by
        ...
        calc
          |chartGramOnE (I := I) p i j y - chartGramOnE (I := I) p i j x0| *
                ...
                |Riemannian.chartCoord (E := E) j u| ≤
              |chartGramOnE (I := I) p i j y - chartGramOnE (I := I) p i j x0| *
                (‖Riemannian.chartCoordFunctional (E := E) i‖ * ‖u‖) *
                (‖Riemannian.chartCoordFunctional (E := E) j‖ * ‖u‖) := by
            refine mul_le_mul (mul_le_mul_of_nonneg_left hi (abs_nonneg _)) hj
              (abs_nonneg _) ?_
            positivity
          _ = |chartGramOnE (I := I) p i j y - chartGramOnE (I := I) p i j x0| *
              (‖Riemannian.chartCoordFunctional (E := E) i‖ *
                ‖Riemannian.chartCoordFunctional (E := E) j‖) * ‖u‖ ^ 2 := by ring
      _ = S y * ‖u‖ ^ 2 := by
        simp only [hS]
        rw [Finset.sum_mul]
        refine Finset.sum_congr rfl fun i _ ↦ ?_
  ...
  have hetaLam : S y * ‖u‖ ^ 2 ≤ eta * (lam * ‖u‖ ^ 2) := by
    nlinarith [mul_le_mul_of_nonneg_right hSy.le (sq_nonneg ‖u‖)]
  have hkey : |chartMetricInner (I := I) p y u u -
      chartMetricInner (I := I) p x0 u u| ≤
      eta * chartMetricInner (I := I) p x0 u u := by
    refine hdev.trans (hetaLam.trans ?_)
    exact mul_le_mul_of_nonneg_left (hlow0 u) heta.le
  obtain ⟨h1, h2⟩ := abs_le.mp hkey
  constructor <;> nlinarith

/-! This is the source's displacement lower bound.  The linear functional `Phi` is the fixed-chart
Gram pairing with the endpoint displacement; the fundamental theorem of calculus turns its
integral into the quadratic form, while the lower Gram comparison controls the moving speed. -/
private theorem ofReal_sqrt_gram_displacement_le_pathELength
    {p : M} {eta delta : ℝ} (heta1 : eta < 1)
    ...
    ENNReal.ofReal (Real.sqrt ((1 - eta) *
        chartMetricInner (I := I) p (extChartAt I p p)
  ...
  have hxC1 : ContDiffOn ℝ 1 x (Icc c d) :=
    contDiffOn_extChartAt_comp hgamma hsrc
  ...
  set Q : ℝ := chartMetricInner (I := I) p x0 w w with hQ
  have hQ0 : 0 ≤ Q :=
    chartMetricInner_self_nonneg_of_mem_target (I := I) p hx0tgt w
  rcases hQ0.eq_or_lt with hQzero | hQpos
  · rw [← hQzero]
    simp
  set Phi : E →L[ℝ] ℝ := ∑ i, ∑ j,
    (chartGramOnE (I := I) p i j x0 * chartCoord (E := E) i w) •
      chartCoordFunctional (E := E) j with hPhi
  have hPhi_apply : ∀ u : E,
      Phi u = chartMetricInner (I := I) p x0 w u := by
    intro u
    rw [hPhi, chartMetricInner, sum_apply]
    refine Finset.sum_congr rfl fun i _ ↦ ?_
    rw [sum_apply]
    refine Finset.sum_congr rfl fun j _ ↦ ?_
    rw [smul_apply, chartCoordFunctional_apply]
    simp only [smul_eq_mul]
  have htgt : ∀ s ∈ Icc c d, x s ∈ (extChartAt I p).target := fun s hs ↦
    hball (hchart s hs)
  have hpt : ∀ s ∈ Ioo c d, Phi (D s) ≤
      (Real.sqrt Q / Real.sqrt (1 - eta)) *
        Real.sqrt (chartMetricInner (I := I) p (x s) (D s) (D s)) := by
    intro s hs
    have hsIcc : s ∈ Icc c d := Ioo_subset_Icc_self hs
    have hCS : Phi (D s) ≤ Real.sqrt Q *
        Real.sqrt (chartMetricInner (I := I) p x0 (D s) (D s)) := by
      rw [hPhi_apply]
      exact chartMetricInner_le_sqrt_mul_sqrt_of_mem_target (I := I) p hx0tgt w (D s)
    have hdom : chartMetricInner (I := I) p x0 (D s) (D s) ≤
        chartMetricInner (I := I) p (x s) (D s) (D s) / (1 - eta) := by
      rw [le_div_iff₀ heta']
      simpa [mul_comm] using hlow (x s) (hchart s hsIcc) (D s)
    have hmoving0 : 0 ≤ chartMetricInner (I := I) p (x s) (D s) (D s) :=
      chartMetricInner_self_nonneg_of_mem_target (I := I) p (htgt s hsIcc) (D s)
    have hsqrt : Real.sqrt (chartMetricInner (I := I) p x0 (D s) (D s)) ≤
        Real.sqrt (chartMetricInner (I := I) p (x s) (D s) (D s)) /
          Real.sqrt (1 - eta) := by
      have h := Real.sqrt_le_sqrt hdom
      rwa [Real.sqrt_div hmoving0] at h
    calc
      Phi (D s) ≤ Real.sqrt Q *
          Real.sqrt (chartMetricInner (I := I) p x0 (D s) (D s)) := hCS
      _ ≤ Real.sqrt Q *
          (Real.sqrt (chartMetricInner (I := I) p (x s) (D s) (D s)) /
            Real.sqrt (1 - eta)) :=
        mul_le_mul_of_nonneg_left hsqrt (Real.sqrt_nonneg Q)
      _ = (Real.sqrt Q / Real.sqrt (1 - eta)) *
          Real.sqrt (chartMetricInner (I := I) p (x s) (D s) (D s)) := by ring
  ...
  have hinnerCont : ContinuousOn
      (fun s => chartMetricInner (I := I) p (x s) (D s) (D s)) (Icc c d) :=
    continuousOn_chartMetricInner_along (I := I) p
      hxC1.continuousOn hDcont hDcont htgt
  have hsqrtInt : IntervalIntegrable
      (fun s => Real.sqrt (chartMetricInner (I := I) p (x s) (D s) (D s)))
      volume c d :=
  ...
  have hstep : Q * Real.sqrt (1 - eta) ≤ Real.sqrt Q *
      ∫ s in c..d, Real.sqrt (chartMetricInner (I := I) p (x s) (D s) (D s)) := by
    have h := mul_le_mul_of_nonneg_right hmono hetaSqrt.le
    calc
      Q * Real.sqrt (1 - eta) ≤
          (Real.sqrt Q / Real.sqrt (1 - eta) *
            ∫ s in c..d, Real.sqrt
              (chartMetricInner (I := I) p (x s) (D s) (D s))) *
              Real.sqrt (1 - eta) := h
      _ = Real.sqrt Q * ∫ s in c..d, Real.sqrt
            (chartMetricInner (I := I) p (x s) (D s) (D s)) := by
        field_simp
  have hfinal : Real.sqrt (1 - eta) * Real.sqrt Q ≤
      ∫ s in c..d, Real.sqrt
        (chartMetricInner (I := I) p (x s) (D s) (D s)) := by
    nlinarith [hstep, Real.mul_self_sqrt hQ0]
  have hbridge := pathELength_eq_ofReal_integral_chartMetricInner
    (I := I) hlt.le hgamma hsrc
  rw [hbridge]
  apply ENNReal.ofReal_le_ofReal
  rw [Real.sqrt_mul heta'.le]
  simpa [hx, hD, hw, hQ] using hfinal

private theorem exists_chartLine_pathELength_le
    {p : M} {eta delta a : ℝ}
    (hball : closedBall (extChartAt I p p) delta ⊆ (extChartAt I p).target)
    ...
    ∃ (sigma : ℝ → M) (J : Set ℝ), IsOpen J ∧
      Icc a (a + 1) ⊆ J ∧
      ContMDiffOn 𝓘(ℝ, ℝ) I ∞ sigma J ∧ sigma a = q0 ∧
      sigma (a + 1) = q1 ∧
  ...
  have hy0tgt : y0 ∈ (extChartAt I p).target := hball (ball_subset_closedBall hy0)
  have hy1tgt : y1 ∈ (extChartAt I p).target := hball (ball_subset_closedBall hy1)
  ...
  have hsigma0 : sigma a = q0 := by
    change (extChartAt I p).symm (line a) = q0
    rw [hline]
    simpa using hq0eq
  have hsigma1 : sigma (a + 1) = q1 := by
    have hline1 : line (a + 1) = y1 := by
      rw [hline, hv]
      module
    change (extChartAt I p).symm (line (a + 1)) = q1
    rw [hline1]
    exact hq1eq
  ...
  have hbridge := pathELength_eq_ofReal_integral_chartMetricInner
    (I := I) (by linarith : a ≤ a + 1) hsigmaC1 hsrc
  have hbase : ∀ t ∈ Icc a (a + 1), extChartAt I p (sigma t) = line t :=
    fun t ht ↦ hread t (hsub ht)
  have hintCont : ContinuousOn
      (fun t => Real.sqrt (chartMetricInner (I := I) p (line t) v v))
      (Icc a (a + 1)) := by
    apply Real.continuous_sqrt.comp_continuousOn
    exact continuousOn_chartMetricInner_along (I := I) p
      hlineCont.continuousOn continuousOn_const continuousOn_const
      (fun t ht => hball (ball_subset_closedBall (hsub ht)))
  have hint : IntervalIntegrable
      (fun t => Real.sqrt (chartMetricInner (I := I) p (line t) v v))
      volume a (a + 1) := hintCont.intervalIntegrable_of_Icc (by linarith)
  have hpoint : ∀ t ∈ Ioo a (a + 1),
      Real.sqrt (chartMetricInner (I := I) p (line t) v v) ≤
        Real.sqrt ((1 + eta) *
          chartMetricInner (I := I) p x0 v v) := by
    intro t ht
    exact Real.sqrt_le_sqrt
  ...
  apply ENNReal.ofReal_le_ofReal
  calc
    (∫ t in a..a + 1, Real.sqrt (chartMetricInner (I := I) p
      (extChartAt I p (sigma t))
      (derivWithin (fun s => extChartAt I p (sigma s)) (Icc a (a + 1)) t)
      (derivWithin (fun s => extChartAt I p (sigma s)) (Icc a (a + 1)) t)))
        = ∫ t in a..a + 1, Real.sqrt (chartMetricInner (I := I) p (line t) v v) := by
    ...
    _ ≤ ∫ _ in a..a + 1,
          Real.sqrt ((1 + eta) * chartMetricInner (I := I) p x0 v v) := hmono
    _ = Real.sqrt ((1 + eta) * chartMetricInner (I := I) p x0 v v) := by simp
    _ = Real.sqrt ((1 + eta) * chartMetricInner (I := I) p (extChartAt I p p)
          (extChartAt I p q1 - extChartAt I p q0)
          (extChartAt I p q1 - extChartAt I p q0)) := by
      rw [hxq0, hxq1, hv, hx0]

private theorem exists_chartLine_pathELength_le_mul
    {p : M} {eta delta out : ℝ} (heta : 0 < eta) (heta1 : eta < 1)
    (hball : closedBall (extChartAt I p p) delta ⊆ (extChartAt I p).target)
    ...
    ∃ (sigma : ℝ → M) (J : Set ℝ), IsOpen J ∧
      Icc out (out + 1) ⊆ J ∧ ContMDiffOn 𝓘(ℝ, ℝ) I ∞ sigma J ∧
  ...
  obtain ⟨sigma, J, hJ, hJsub, hsigma, hsigma0, hsigma1, hsigmaLen⟩ :=
    exists_chartLine_pathELength_le (I := I) (p := p) (eta := eta) (delta := delta)
      (a := out) hball (fun y hy u ↦ (hcomp y hy u).2)
      (hmem a ⟨le_rfl, hab⟩) (hmem b ⟨hab, le_rfl⟩)
  refine ⟨sigma, J, hJ, hJsub, hsigma, hsigma0, hsigma1, ?_⟩
  have hlower := ofReal_sqrt_gram_displacement_le_pathELength
    (I := I) heta1 hball (fun y hy u ↦ (hcomp y hy u).1)
      hab hgamma (fun s hs ↦ image_mono ball_subset_closedBall (hmem s hs))
  set Q : ℝ := chartMetricInner (I := I) p (extChartAt I p p)
...
omit [FiniteDimensional ℝ E] [IsManifold I ∞ M] [I.Boundaryless]
    [IsContMDiffRiemannianBundle I ∞ E (fun x : M ↦ TangentSpace I x)] in
private theorem exists_polygon_glue {C : ℝ≥0∞}
    {gamma : ℝ → M} {t : ℕ → ℝ} {N : ℕ}
    (hmono : ∀ i < N, t i ≤ t (i + 1))
    (hpieces : ∀ i < N,
      ∃ (sigma : ℝ → M) (J : Set ℝ), IsOpen J ∧
  ...
  have key : ∀ k, k ≤ N →
    ...
    | succ k ih =>
        ...
        have hP'eqSigma : EqOn P' sigma (Icc (k : ℝ) ((k : ℝ) + 1)) := by
          ...
          · rw [← heq, ite_eq_left le_rfl, hPk, hsigma0]
...
/-- **Chart-polygon approximation.** Every `C¹` path on `[0,1]` admits a finite, strictly
increasing sequence of selected parameters with smooth pieces on the selected subintervals; the
first and last selected values match the path endpoints, with at most the stated multiplicative
loss in length.  This is the source declaration
`Riemannian.exists_piecewiseSmooth_pathELength_le` in
`DoCarmoLib/Riemannian/Metric/SmoothPathApproximation.lean`, revision
`24f32e4d600878bfaac6bc2f2f9324175571c321` (`[poincareConjectureDoCarmo]`), adapted to the
canonical `RiemannianBundle`/`pathELength` API.  Finite-dimensionality is used for the local
coercivity estimate; neither positive dimension nor `IsRiemannianManifold` is needed. -/
theorem _root_.Riemannian.exists_piecewiseSmooth_pathELength_le
    {gamma : ℝ → M}
  ...
  have hchart : ∀ s : Icc (0 : ℝ) 1, ∃ delta > (0 : ℝ),
      ...
      ∀ y ∈ closedBall (extChartAt I (gamma s) (gamma s)) delta, ∀ u : E,
        (1 - eta) * chartMetricInner (I := I) (gamma s)
            (extChartAt I (gamma s) (gamma s)) u u ≤
            chartMetricInner (I := I) (gamma s) y u u ∧
          chartMetricInner (I := I) (gamma s) y u u ≤
            (1 + eta) * chartMetricInner (I := I) (gamma s)
              (extChartAt I (gamma s) (gamma s)) u u :=
    fun s => exists_chartBall_gram_comparison (I := I) (gamma s) heta
    ...
    have hxmem : x ∈ gamma ⁻¹'
        ((extChartAt I (gamma (⟨x, hx⟩ : Icc (0 : ℝ) 1))).source ∩
          extChartAt I (gamma (⟨x, hx⟩ : Icc (0 : ℝ) 1)) ⁻¹'
            ball (extChartAt I (gamma (⟨x, hx⟩ : Icc (0 : ℝ) 1))
              (gamma (⟨x, hx⟩ : Icc (0 : ℝ) 1)))
              (delta ⟨x, hx⟩)) ∩ Icc 0 1 :=
      ⟨⟨mem_extChartAt_source _, mem_ball_self (hdelta ⟨x, hx⟩)⟩, hx⟩
  ...
  have hpieces : ∀ i < N,
    ...
    obtain ⟨sigma, J, hJ, hJsub, hsigma, hsigma0, hsigma1, hsigmaLen⟩ :=
      exists_chartLine_pathELength_le_mul (I := I) heta heta1
        (hball s) (hcomp s) (hmono i hi) (hgamma.mono hsubIcc) hs
        (out := (i : ℝ))
    refine ⟨sigma, J, hJ, hJsub, hsigma, hsigma0, hsigma1, ?_⟩
    rwa [hratio] at hsigmaLen
  obtain ⟨P, hPsmooth, hP0, hPN, hPlen⟩ :=
    exists_polygon_glue (I := I) (C := ENNReal.ofReal (Real.sqrt (1 + epsilon)))
      hmono hpieces
  rw [ht0] at hP0 hPlen
  rw [htN] at hPN hPlen
